...
|| `doctor` | **Aggregate health diagnostics in one pass**: config in force, index freshness (sources + docs), pack resolution, vector-lane usability and embedding coverage, eval-expectation validity, contract-gate outcome, lint counts. First stop on any new machine or "why is retrieval odd?" moment; `--json`, exits non-zero on error-level checks. |
