...
import { DAMI_UPDATE_LOCK_PATH } from './types.js';
 ...
 * Honor `DAMI_NPM_REGISTRY` env var for manual override (useful for testing
 * or private mirrors).
 */
export function resolveRegistryForPackage(pkgName: string): string {
  const override = process.env.DAMI_NPM_REGISTRY?.trim();
...
export async function acquireLock(lockPath?: string): Promise<boolean> {
  const resolved = lockPath ?? expandHome(DAMI_UPDATE_LOCK_PATH);
  try {
...
export async function doUpdate(): Promise<void> {
  ...
  if (policy === 'prompt') {
    if (!process.stdin.isTTY) {
      log.info(`Update available: v${result.current} → v${result.latest}. Run "dami-harness update" to upgrade.`);
      return;
    }
  ...
  try {
    ...
    log.success(`Updated dami-harness to v${result.latest}`);

    // Refresh hooks using new version's code (spawn new process so updated code is loaded)
    try {
      await execFileAsync('dami-harness', ['hooks', 'inject', '--silent'], {
        timeout: 15_000,
      });
    ...
    if (msg.includes('EACCES') || error.code === 'EACCES') {
      log.warn(`Permission denied. Run "dami-harness update" manually with appropriate permissions.`);
    } else if (msg.includes('ETIMEDOUT') || msg.includes('timed out')) {
      log.warn('Update timed out. Try again later.');
    } else {
      log.warn(`Update failed: ${msg}. Run "dami-harness update" manually.`);
    }
  } finally {
    await releaseLock();
  }
...
/**
 * Main entry point for `dami-harness update` command.
