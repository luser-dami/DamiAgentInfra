...
//  on each `dami-harness pull`.
//
//  dami-harness-recall.md instructs the AI to proactively search the team
//  knowledge base (via the `dami-harness-recall` subagent or `dami-harness recall`)
...
export const BUILTIN_RULE_NAMES = new Set<string>(['dami-harness-recall']);

/** Names of previously deployed rules that should be cleaned up. */
export const LEGACY_RULE_NAMES: string[] = [];

/**
 * Names that scanLocalForCollect and stale-cleanup should skip.
...
export async function deployBuiltinRules(
    ...
    const builtinRules: Array<{ name: string; content: string }> = [
        { name: 'dami-harness-recall', content: DAMI_RECALL_RULE_CONTENT },
    ].filter(r => !(options?.skipRecall && r.name === 'dami-harness-recall'));
...
const DAMI_RECALL_RULE_CONTENT = `# Team Knowledge Recall (dami-harness)
...
调用 \`dami-harness-recall\` subagent（位于 agents/ 目录），传入任务的自然语言描述。
Subagent 会返回结构化的团队知识摘要（skills、learnings、docs、rules、codebase）。

### 方式二：通过 Bash 命令（适用于所有工具）

\`\`\`bash
dami-harness recall "<关键词1> <关键词2> ..."
\`\`\`
...
dami-harness-recall subagent 的返回里已列出本次检索到的候选 doc-id（见其 recalled-doc-ids 列表）。
你**不需要凭记忆重建列表**——只需在**最终回复末尾**追加下面这行，
从候选里**保留你实际用到的**（照抄了命令、或概念上参考了，都算用到），删掉没用到的：

\`\`\`
<!-- dami-harness:referenced-doc-ids: [你实际用到的-doc-id] -->
\`\`\`

一个都没用到就留空：\`<!-- dami-harness:referenced-doc-ids: [] -->\`。
若直接用 \`dami-harness recall\` 命令（未走 subagent），从召回结果的 File 路径推出 doc-id 自行填入。
`;
