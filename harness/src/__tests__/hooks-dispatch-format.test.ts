...
describe('hooks — merged dispatch format', () => {
  ...
  describe('Claude format', () => {
    it('uses hook-dispatch commands instead of individual subcommands', async () => {
      ...
      const dispatchCommands = allCommands.filter((c) => c.includes('dami-harness hook-dispatch'));
      expect(dispatchCommands.length).toBe(allCommands.length);
      // No individual subcommands like "dami-harness pull", "dami-harness track", etc.
      const legacyCommands = allCommands.filter(
        (c) => c.includes('dami-harness pull') || c.includes('dami-harness track') || c.includes('dami-harness dashboard-report'),
      );
  ...
  describe('migration — legacy cleanup', () => {
      ...
      mockFiles['/test/settings.json'] = {
        hooks: {
          SessionStart: [
            {
              matcher: '*',
              hooks: [{ type: 'command', command: 'bash -lc "dami-harness pull 2>/dev/null" || true' }],
              description: '[dami-harness] Auto-pull team resources on session start',
            },
            {
              matcher: '*',
              hooks: [{ type: 'command', command: 'bash -lc "dami-harness dashboard-report --stdin --tool claude 2>/dev/null" || true' }],
              description: '[dami-harness] Dashboard report on session start',
            },
          ],
        },
      };
    ...
    it('preserves non-dami-harness hooks during migration', async () => {
      mockFiles['/test/settings.json'] = {
        hooks: {
          SessionStart: [
            {
