...
vi.mock('../utils/git.js', () => ({
  ...
  generateBranchName: vi.fn().mockReturnValue('dami-harness/push/test/20260305-120000'),
}));
...
describe('RulesHandler.scanLocalForCollect — modified rule detection', () => {
  let tmpDir: string;
  let homeDir: string;
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-rules-test-'));
...
scope: 'user',
  ...
  it('should NOT include built-in rules (dami-harness-recall) in push candidates', async () => {
    await fse.writeFile(path.join(homeDir, '.claude/rules', 'dami-harness-recall.md'), 'auto-generated recall rule');

    const items = await handler.scanLocalForCollect(teamConfig, localConfig);
    const names = items.map((i) => i.name);
    expect(names).not.toContain('dami-harness-recall');
  });
});

describe('RulesHandler.scanLocalForCollect — subdirectory support', () => {
  let tmpDir: string;
  let homeDir: string;
  let handler: RulesHandler;
  let teamConfig: HarnessConfig;
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-rules-subdir-'));
    homeDir = path.join(tmpDir, 'home');
...
describe('RulesHandler.scanStoreForInstall — subdirectory support', () => {
  let tmpDir: string;
  let homeDir: string;
  let handler: RulesHandler;
  let teamConfig: HarnessConfig;
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-rules-pull-subdir-'));
    homeDir = path.join(tmpDir, 'home');
...
describe('RulesHandler.pullAllRules — stale file cleanup', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-rules-stale-'));
    homeDir = path.join(tmpDir, 'home');
...
scope: 'user',
  ...
  it('should not remove built-in rules (dami-harness-recall) during stale cleanup', async () => {
    const teamRulesDir = path.join(localConfig.repo.localPath, 'rules');
    ...
    await fse.writeFile(path.join(localRulesDir, 'dami-harness-recall.md'), 'recall rule content');
    ...
    expect(await fse.pathExists(path.join(localRulesDir, 'dami-harness-recall.md'))).toBe(true);
