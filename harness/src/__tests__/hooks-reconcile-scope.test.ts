...
function manifest(): Promise<Record<string, Array<{ id: string }>>> {
  return fse.readJson(path.join(project, '.dami-harness', 'managed-hooks.json'));
}

beforeEach(async () => {
  project = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-recon-proj-'));
  repo = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-recon-repo-'));
...
`);
    ...
    expect(claude.hooks.Stop[1].description).toBe('[dami-harness:hook:lint] run lint at stop');
...
`);
    ...
    expect(claude.hooks.Stop[0].description?.startsWith('[dami-harness] ')).toBe(true);
...
`);
  ...
  it('works with no hooks.yaml (built-in self-heal only)', async () => {
    ...
    expect(await fse.pathExists(path.join(project, '.dami-harness', 'managed-hooks.json'))).toBe(false);
  });
});
