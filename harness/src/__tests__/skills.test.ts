...
describe('SkillsHandler.scanLocalForCollect', () => {
  let tmpDir: string;
  let homeDir: string;
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-skills-test-'));
...
scope: 'user',
  ...
  it('should not detect a new skill listed in ~/.dami-harness/pushignore', async () => {
    ...
    await fse.writeFile(path.join(homeDir, '.dami-harness', 'pushignore'), 'ignored-skill\n');
  ...
  it('should not detect a modified skill listed in ~/.dami-harness/pushignore', async () => {
    ...
    await fse.ensureDir(path.join(homeDir, '.dami-harness'));
    await fse.writeFile(path.join(homeDir, '.dami-harness', 'pushignore'), '\n  ignored-skill  \n');
...
describe('SkillsHandler.collectItem', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-skills-push-'));
    homeDir = path.join(tmpDir, 'home');
...
scope: 'user',
  ...
  it('should create CONTRIBUTORS file with current user on first push', async () => {
    ...
    await handler.collectItem(item, teamConfig, localConfig);
...
describe('SkillsHandler.readContributors', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-contrib-'));
  });
...
describe('scanTeamRepoNamespaces', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-ns-'));
  });
...
describe('ensureSkillFrontmatter', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-frontmatter-'));
  });
  ...
  it('collectItem auto-injects frontmatter for skills without it', async () => {
...
describe('SkillsHandler.installItem honors disabledAgents', () => {
  let tmpDir: string;
  let homeDir: string;
  let handler: SkillsHandler;
  let teamConfig: HarnessConfig;
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-skills-pull-disabled-'));
    homeDir = path.join(tmpDir, 'home');
    const repoPath = path.join(tmpDir, 'team-repo');
