...
function extractHarnessSubcommands(hooks: Record<string, unknown[]>): string[] {
  const cmds = extractCommands(hooks);
  const subcmds = new Set<string>();
  for (const cmd of cmds) {
    const match = cmd.match(/dami-harness\s+([\w-]+)/);
...
describe('hooks', () => {
  ...
  describe('inject — idempotency', () => {
      ...
      mockFiles['/test/settings.json'] = {
        hooks: {
          SessionStart: [
            {
              matcher: '*',
              hooks: [{ type: 'command', command: 'bash -lc "dami-harness pull --silent" 2>/dev/null || true' }],
              description: '[dami-harness] Auto-pull team resources on session start',
            },
          ],
        },
      };
    ...
    it('updates command when content changes (Cursor)', async () => {
      ...
      mockFiles['/test/hooks.json'] = {
        ...
        hooks: {
          sessionStart: [
            { command: 'bash -lc "dami-harness pull --silent" 2>/dev/null || true', timeout: 30 },
          ],
        },
      };
  ...
  describe('inject — preserves non-dami-harness hooks', () => {
  ...
  describe('remove', () => {
    it('Claude format: removes all dami-harness hooks, preserves others', async () => {
      await injectHooks('/test/settings.json', 'claude');
    ...
    it('Cursor format: removes all dami-harness hooks, preserves others', async () => {
      await injectHooks('/test/hooks.json', 'cursor');
  ...
  describe('inject — stale event key cleanup', () => {
    it('Cursor inject removes stale dami-harness event keys (e.g. userPromptSubmit)', async () => {
      mockFiles['/test/hooks.json'] = {
        version: 1,
        hooks: {
          userPromptSubmit: [
            { command: 'bash -lc "dami-harness track-slash --stdin --tool cursor 2>/dev/null" || true', timeout: 10 },
            { command: 'bash -lc "dami-harness dashboard-report --stdin --tool cursor 2>/dev/null" || true', timeout: 10 },
          ],
        },
      };
  ...
  describe('format alignment', () => {
    it('Claude and Cursor inject the same set of dami-harness subcommands (except Claude-only hooks)', async () => {
      ...
      const claudeSubcmds = extractHarnessSubcommands(claudeResult.hooks);
      const cursorSubcmds = extractHarnessSubcommands(cursorResult.hooks);

      // Both Claude and Cursor should have the same subcommands
      expect(claudeSubcmds).toEqual([...DAMI_HOOK_SUBCOMMANDS].sort());
      // Cursor should also have all subcommands (contribute-check moved to Stop, supported by both)
      expect(cursorSubcmds).toEqual([...DAMI_HOOK_SUBCOMMANDS].sort());
    });
    ...
    it('Claude hooks have [dami-harness] description prefix', async () => {
      await injectHooks('/test/settings.json', 'claude');
      const result = mockFiles['/test/settings.json'] as { hooks: Record<string, Array<{ description?: string }>> };
      for (const entries of Object.values(result.hooks)) {
        for (const entry of entries) {
          expect(entry.description).toMatch(/^\[dami-harness\]/);
        }
      }
    });
  ...
  describe('DAMI_HOOK_SUBCOMMANDS export', () => {
    ...
    it('DAMI_LEGACY_HOOK_SUBCOMMANDS contains all old subcommands for cleanup', () => {
      expect(DAMI_LEGACY_HOOK_SUBCOMMANDS).toContain('pull');
      expect(DAMI_LEGACY_HOOK_SUBCOMMANDS).toContain('update');
      expect(DAMI_LEGACY_HOOK_SUBCOMMANDS).toContain('track');
      expect(DAMI_LEGACY_HOOK_SUBCOMMANDS).toContain('track-slash');
      expect(DAMI_LEGACY_HOOK_SUBCOMMANDS).toContain('dashboard-report');
      expect(DAMI_LEGACY_HOOK_SUBCOMMANDS).toContain('contribute-check');
      expect(DAMI_LEGACY_HOOK_SUBCOMMANDS).toContain('auto-recall');
    });
  });
  ...
  describe('getHookStatus', () => {
    ...
    it('reports missing when settings exist without dami-harness hooks', async () => {
