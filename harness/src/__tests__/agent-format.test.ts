...
function buildTeamConfig(toolPaths: HarnessConfig['toolPaths']): HarnessConfig {
  return {
    team: 'test',
    description: '',
...
// ─── AgentsHandler.collectItem — skip path ──────────────────────────────────────

describe('AgentsHandler.collectItem — skipReason path', () => {
  let tmpDir: string;
  let repoPath: string;
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-agents-push-test-'));
...
// ─── AgentsHandler.installItem — multi-target ───────────────────────────────────

describe('AgentsHandler.installItem — multi-target', () => {
  let tmpDir: string;
  let homeDir: string;
  let repoPath: string;
  let handler: AgentsHandler;
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-agents-pull-test-'));
