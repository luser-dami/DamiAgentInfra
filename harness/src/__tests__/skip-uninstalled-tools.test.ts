...
describe('ResourceHandler.isToolInstalled', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-install-test-'));
...
describe('SkillsHandler.installItem — skip uninstalled tools', () => {
  let tmpDir: string;
  let homeDir: string;
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-skills-pull-'));
    homeDir = path.join(tmpDir, 'home');
...
describe('RulesHandler.installItem — skip uninstalled tools', () => {
  let tmpDir: string;
  let homeDir: string;
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-rules-pull-'));
    homeDir = path.join(tmpDir, 'home');
...
describe('RulesHandler.pullAllRules — skip CLAUDE.md update for uninstalled tools', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-claudemd-'));
    homeDir = path.join(tmpDir, 'home');
...
describe('deployBuiltinSkills — skip uninstalled tools', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fse.mkdtemp(path.join(os.tmpdir(), 'dami-harness-builtin-skills-'));
    homeDir = path.join(tmpDir, 'home');
    ...
    builtinSkillsDir = path.join(tmpDir, 'builtin-skills', 'dami-harness-test-skill');
