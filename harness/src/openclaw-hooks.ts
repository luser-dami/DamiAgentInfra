 ...
 * same `dami-harness hook-dispatch` entry point.
 *
...
/** Sub-directory name under <hooksDir> that holds the dami-harness OpenClaw hook. */
export const OPENCLAW_HOOK_DIR = 'dami-harness-status-report';

/** Marker so we can recognize (and cleanly remove) our own hook. */
const DAMI_MARKER = '[dami-harness]';

/** Map OpenClaw event → dami-harness dispatch event. */
...
function buildHookMd(tool: string): string {
  ...
  return [
    '---',
    `name: ${DAMI_MARKER} status-report`,
    `metadata:`,
    `  ${metadata}`,
    `handler: ./handler.ts`,
    '---',
    '',
    `${DAMI_MARKER} Reports agent status to the team backend (report/sync/ack) for tool \`${tool}\`.`,
    'Managed by dami-harness — do not edit by hand.',
    '',
  ].join('\n');
}

function buildHandlerTs(tool: string): string {
  // Map each OpenClaw event to the corresponding dami-harness dispatch event and shell
  // out. Failures are swallowed so the agent is never blocked.
  const mapLiteral = JSON.stringify(EVENT_MAP);
  return `// ${DAMI_MARKER} status-report handler — generated by dami-harness, do not edit.
...
export default async function handler(ctx: { event?: string } = {}): Promise<void> {
  ...
  try {
    const child = spawn('dami-harness', ['hook-dispatch', dispatchEvent, '--tool', TOOL], {
...
/**
 * Inject (or refresh) the dami-harness OpenClaw hook into `<hooksDir>`.
...
export async function injectOpenClawHooks(hooksDir: string, tool = 'openclaw'): Promise<void> {
  ...
  log.success(`Injected dami-harness OpenClaw hook into ${dir}`);
}

/** Remove the dami-harness OpenClaw hook from `<hooksDir>` if present. */
export async function removeOpenClawHooks(hooksDir: string): Promise<void> {
  const dir = path.join(hooksDir, OPENCLAW_HOOK_DIR);
  if (await pathExists(dir)) {
    await remove(dir);
    log.success(`Removed dami-harness OpenClaw hook from ${dir}`);
  }
  ...
  if (process.env.OPENCLAW_STATE_DIR) {
    ...
    if (altDir !== dir && await pathExists(altDir)) {
      await remove(altDir);
      log.success(`Removed dami-harness OpenClaw hook from ${altDir}`);
    }
  }
}
...
function buildAgentHookMd(slug: string, openclawEvent: string): string {
  ...
  return [
    '---',
    `name: ${DAMI_MARKER} ${slug}`,
    `metadata:`,
    `  ${metadata}`,
    `handler: ./handler.ts`,
    '---',
    '',
    `${DAMI_MARKER} Agent hook [${slug}] — managed by dami-harness, do not edit by hand.`,
    '',
  ].join('\n');
}
...
function buildAgentHandlerTs(command: string, timeout: number): string {
  ...
  return [
    `// ${DAMI_MARKER} agent hook handler — generated by dami-harness, do not edit.`,
    `import { spawn } from 'node:child_process';`,
    '',
