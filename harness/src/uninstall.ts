...
interface RemovalPlan {
  ...
  ruleFiles: string[];
  /** Built-in agent .md files deployed by the CLI (e.g. dami-harness-recall). */
  agentFiles: string[];
  ...
  harnessHome: string;
  /** Whether harnessHome exists on disk. */
  harnessHomeExists: boolean;
...
/** Detect hooks cleared to empty arrays — a residue of prior dami-harness installation. */
...
async function discoverToolResources(
  tool: string,
  toolPath: HarnessConfig['toolPaths'][string],
  baseDir: string,
  ...
  // (b) CLAUDE.md dami-harness section blocks
  ...
  // (d2) Built-in agents — CLI-deployed subagents (e.g. dami-harness-recall).
...
async function buildRemovalPlan(
  ...
  if (agentFilter) {
    ...
    // Other tools still have dami-harness resources → keep shared resources.
    ...
    // the last tool using dami-harness. Targeting a tool with no dami-harness resources is a
  ...
  if (includeShared) {
    // (d3) dami-harness-managed MCP servers, tracked in managed-mcp.json (same
    // ownership model as hooks). These live under ~/.dami-harness, so they are shared
    // resources: only removed when the target is the last tool using dami-harness.
...
function printSummary(plan: RemovalPlan, agentFilter?: string): void {
  ...
  console.log('');
  console.log(`⚠  正在卸载 ${plan.scope} scope（${cn}）— ${plan.harnessHome}`);
  if (agentFilter) {
  ...
  console.log('⚠  以下 dami-harness 资源将被移除:');
  console.log('');
  ...
  if (plan.harnessHomeExists) {
    console.log('   DamiHarness 主目录:');
    console.log(`     ${plan.harnessHome}/`);
    console.log('');
  }
}
...
async function executeRemoval(plan: RemovalPlan): Promise<void> {
  ...
  // (b) Clean CLAUDE.md dami-harness section blocks
  ...
  // (d2) Remove built-in agent files (e.g. dami-harness-recall)
  ...
  // (g) Remove ~/.dami-harness/ directory (last — earlier steps read from it)
  if (plan.harnessHomeExists) {
    try {
      await remove(plan.harnessHome);
...
export async function uninstall(opts: UninstallOptions): Promise<void> {
  ...
  try {
    ...
    log.warn('dami-harness 配置未找到或无效');
  }
  ...
  if (localConfig && teamConfig) {
    ...
    // MCP cleanup must run before executeRemoval deletes ~/.dami-harness/: ownership is
    ...
    if (plan.includeShared) {
      try {
        ...
        if (removed.length > 0) log.info(`Removed ${removed.length} dami-harness-managed MCP server(s)`);
      } catch (e) {
    ...
    // shared ~/.dami-harness home survives (non-last-tool uninstall); on a last-tool
    ...
    log.success('dami-harness 卸载完成');
  } else {
    // Minimal uninstall — just try to remove ~/.dami-harness/
    if (opts.agent) {
      log.warn('No valid dami-harness configuration detected; cannot target a specific tool with --agent');
      process.exitCode = 2;
      return;
    }
    ...
    console.log('⚠  将移除 DamiHarness 主目录:');
    console.log(`     ${home}/`);
    console.log('');
