...
//
//  CLI ships with built-in skills (e.g. dami-harness-contribute).
...
//
//  npm package
//    skills/dami-harness-contribute/SKILL.md
//      │
...
export const BUILTIN_SKILL_NAMES = new Set(['dami-harness-share-learnings', 'team-wiki-codebase', 'dami-harness-workflow', 'dami-harness-import']);

/** Built-in skills that depend on recall being enabled. Skipped when recall is disabled. */
export const RECALL_DEPENDENT_SKILLS = new Set(['dami-harness-share-learnings', 'team-wiki-codebase']);
 ...
 * to every tool's skills path defined in dami-harness.yaml.
 *
 * Silently skips if:
 * - Built-in skills directory doesn't exist (dev environment without build)
 * - A tool's skills directory is not configured
 */
export async function deployBuiltinSkills(teamConfig: HarnessConfig, localConfig?: LocalConfig, options?: { reportingOnly?: boolean; skipRecall?: boolean }): Promise<number> {
  // Reporting-only HTTP mode has no team repo to write to, so the team-repo-
  // dependent built-in skill (dami-harness-share-learnings) is non-functional there.
  // Skip built-in skills entirely.
  if (options?.reportingOnly) {
    log.debug('Reporting-only mode (no team repo): skipping built-in skills (dami-harness-share-learnings)');
    return 0;
  }
