...
import { DAMI_RULES_START, DAMI_RULES_END, resolveBaseDir, isAgentDisabled } from '../types.js';
...
export class RulesHandler extends ResourceHandler {
  ...
  async pullAllRules(
    ...
    for (const rule of rules) {
      await this.installItem(rule, teamConfig, localConfig);
    }
    ...
    for (const [, toolPath] of Object.entries(teamConfig.toolPaths)) {
      ...
      try {
        ...
        if (!content || !content.includes(DAMI_RULES_START)) continue;
        const startIdx = content.indexOf(DAMI_RULES_START);
        const endIdx = content.indexOf(DAMI_RULES_END);
        if (startIdx === -1 || endIdx === -1) continue;
        const before = content.substring(0, startIdx).replace(/\n+$/, '\n');
        const after = content.substring(endIdx + DAMI_RULES_END.length).replace(/^\n+/, '\n');
